...
BEGIN declare newid int;
	...
	if
(
	select count(invoice.id) from invoice where category.code = _code) > 0 then
	SET @message_text = CONCAT('Catgory \'', name, '\' already exists');
	SIGNAL
	SQLSTATE '45000' SET MESSAGE_TEXT = @message_text;
else
		insert into invoice(code, type, product_id, qty, price, to_date, from_date, active_flag, created_date, updated_date)
        values (_code, _type, _product_id, _qty, _price, _to_date, _from_date, _active_flag, _created_date, _updated_date);
...
body: begin
update invoice
set code = _code, type= _type, product_id =_product_id, qty = _qty, price= _price, 
to_date= _to_date, from_date=_from_date, active_flag = _active_flag, updated_date = _updated_date;
END$$
DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE invoice_findByCode(  _code nvarchar(50))
begin
select * from invoice where active_flag = 1 or  active_flag = 0 and code like '%' || _code || '%';
end$$
DELIMITER ;