...
body:BEGIN
	...
	if(select count(menu.id) from menu where menu.name) > 0 then
...
body:begin
	update menu set  parent_id = _parent_id,url =_url, name = _name,  order_index= _order_index, active_flag = _active_flag, updated_date = _updated_date;
END$$
DELIMITER ;
...
begin
	select * from menu where active_flag = 1 or active_flag = 0 ;
end$$
DELIMITER ;
