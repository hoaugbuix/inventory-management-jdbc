...
BEGIN
    ...
    --  if(select count(h.id) from history h where id is not null) > 0 then
--         -- SET @message_text = CONCAT('history \'', _action_name, '\' already exists');
--         set @message_text = '';
--         SIGNAL
...
begin
    select h.*, p.*
    from history h
             inner join product_info p on h.product_id = id
...
drop procedure if EXISTS history_findById;
DELIMITER $$
CREATE PROCEDURE history_findById(in _id int)
begin
    select *
    from history
    where id = _id
