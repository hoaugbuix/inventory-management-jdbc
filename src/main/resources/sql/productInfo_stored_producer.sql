...
body:BEGIN
	...
	if(select count(product_info.id) from product_info where product_info.name and product_info.code) > 0 then
...
body:begin
	update product_info set  code = _code, name = _name,  description= _description,img_url = _img_url, cate_id = _cate_id, active_flag = _active_flag, updated_date = _updated_date;
END$$
DELIMITER ;
